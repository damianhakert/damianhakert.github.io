...
1. People Ops Generalist pings the People Ops Specialist to determine a compensation range and submit a proposal to the [compensation committee](/handbook/people-operations/global-compensation-calculator/#compensation-committee) for approval. For any position, filling in the NYC benchmark salary in the `jobs.yml` file will automatically cause the [Compensation Calculator](/handbook/people-operations/global-compensation-calculator) to show at the bottom of the job description page. People Ops Generalist discusses with hiring manager whether this is a position for which the search is fully open to any location, or whether it can be more directed to encourage g Globally, xx % of people live in metro areas with a rent index greater than yy, while at GitLab, this is zz % of our Developers. Therefore, we are now focusing on only hiring in metro areas with a **rent index lower than yy** to add to our [diverse workforce](/handbook/values) while staying [frugal](/handbook/values). Please bear this in mind when using the compensation calculator below.   (which then usually automatically excludes some high cost metro areas) or not. If the decision is that there _should_  be a limit on rent index, then this needs to be included specifically in the job description, with clarifying text. 
