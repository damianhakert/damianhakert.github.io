...
1. When signing up for a new service ask yourself which team members you need to
   **share access** with. There are three types of account access for these services:
   ...
   1. OAuth accounts (authentication through GitLab or Google, such as for checkmk monitoring and grafana).

The 'Shared' vault (to which all team members have access) has a secure note for _each_ service that uses individual or OAuth account access. The note lists whom you should contact to gain access to the service. You should be added to most of these services during onboarding.
