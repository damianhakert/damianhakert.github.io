...
- `Rent Index` is taken from [Numbeo](https://www.numbeo.com/cost-of-living/rankings.jsp), which expresses the ratio of cost of rent in many metro areas to the average cost of rent in New York City (i.e. Rent Index in New York = 1.00)
...
It was a small step to go from the initial linear regression to picking the coefficients that are now in the formula, except that we 'discovered' that an offset was needed in the Rent Index to make things work (i.e. to have the formula 'predict' compensations that were in line with current actual compensations). As a consequence of this offset of 25%, for an employee in New York their median compensation will be 25% _higher_ than what the New York benchmark would have suggested. This may reflect that we don't generally hire 'median' performers.

The contract type factor helps to make the distinction between an employee and a contractor, and also takes into account international differences when it comes to employee compensation. For example, a typical contractor may have to bear the costs of their own health insurance, social security taxes and so forth, leading to a higher compensation for the contractor. For our international employees, in some countries the cost to the employer of providing benefits and contributing to government funded social protections is greater than in others. For example, this would lead to an employee in the Netherlands receiving a lower paycheck than an employee in the USA under otherwise equal circumstances (with the benefits of being in the Netherlands though!). The contract type factor is meant to capture these differences.
