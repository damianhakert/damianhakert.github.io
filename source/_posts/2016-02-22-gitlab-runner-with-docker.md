...
In this post, we are using the docker executor. Remember from the gitlab-ci.yml file, our base image is node:4.2.2.
...
caching and restoring dependencies between builds. For NodeJS projects, dependent 
libraries are installed in a folder called `node_modules`. 
...
After a successful build, you should see `node_modules` folder is archived at 
...
parallelize the tests. In our project, we can split the tests into two jobs. One for the database tests and another for the async module tests. Furthermore, we can restrict the PostgreSQL service to run only for our database job since we won't need a database for the async module.
